...
function traversingSchema(schema, key) {
  ...
  if (schema.isJoi) {
    ...
    // schema is wrong
    throw new Error(
      `schema is invalid with key: ${key}, got ${JSON.stringify(schema)}`
    );
  }
  return result;
}
    ...
    m("td", [
      ...
      tests.length > 0
        ? `, ${tests
            ...
            .join(",")}`
        : "",
      valids.size ? `, in [${[...valids.values()]}]` : "",
      invalids.size > 1 ? `, not in [${[...invalids.values()]}]` : ""
    ]),
    m("td", schema._examples.map(ex => ex.value).join(", ")),
